...
def forward(initial_p, name, record=False, annotate=False):    
    ...
    J  = det(F)
    I1 = tr(C)
    psi = (mu/2.)*(I1-2.)-mu*ln(J)+(lmbda/2.)*(ln(J))**2
    Pi          = psi*dx-dot(beta*grad(p_n),u)*dx
    F_HE        = derivative(Pi, u, v)
    ...
    s = sigma - (1./2)*tr(sigma)*I  # deviatoric stress
    von_Mises = sqrt(3./2*inner(s, s))
    vm = project(von_Mises, V, annotate=annotate)
...
output_dir = './output/rat05/minhe2'
...
beta   = Constant(.01)     # force coefficient for HE
#k0     = Constant(2.)     # growth rate initial guess
k = project(target_p*Constant(5.0),V) #project(k0,V)
